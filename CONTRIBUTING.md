...
5. Add a `new Baseline(X, Y, Z)` entry to `CORE_BASELINES` in [`VersionModules`](Plugin/src/main/java/de/corneliusmay/silkspawners/plugin/version/VersionModules.java). The `vX_Y_Z` module name is derived from the baseline (a trailing `.0` patch is dropped, e.g. `1.16.0` → `v1_16`), so it always matches the module you registered. The highest baseline the server satisfies wins.

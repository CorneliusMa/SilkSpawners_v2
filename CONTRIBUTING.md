...
* **The server version decides.** Version-specific implementations live in modules under [`versions/`](versions) and are resolved through the baseline table in [`MinecraftVersionChecker`](Plugin/src/main/java/de/corneliusmay/silkspawners/plugin/version/MinecraftVersionChecker.java), see [Adding support for a new Minecraft version](#adding-support-for-a-new-minecraft-version).

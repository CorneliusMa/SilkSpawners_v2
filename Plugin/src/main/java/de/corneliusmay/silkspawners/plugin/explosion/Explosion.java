...
public class Explosion {
    ...
    public Explosion(ConfigKey<List<ExplosionTier>> tierList, ConfigKey<List<ExplosionTier>> sharedTierList) {
        this.tiers = combined(tierList.get(), sharedTierList.get());
    }
    ...
    private void explode(Player player, World world, Location location, SpawnerSnapshot spawner, ExplosionTier tier) {
        SpawnerExplodeEvent event =
                new SpawnerExplodeEvent(player, spawner, location, tier.power(), tier.setFire(), tier.breakBlocks());

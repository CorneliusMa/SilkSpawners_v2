...
public class TrialSpawnerFactory implements Loader {
    ...
    public boolean load() {
        if (!isSupported() && config.TRIAL_SPAWNER_ENABLED.get())
            logger.warn("trialspawner.enabled is set, but this server has no trial spawner support"
                    + " (requires Paper 1.21.4 or newer)");
        return true;
    }
    ...
    public EntityType spawnEggEntityType(ItemStack itemStack) {
        return isEnabled() && itemStack != null ? versionAdapter.spawnEggEntityType(itemStack) : null;
    }

    public Optional<TrialSpawner> fromBlock(Block block) {
        if (!isEnabled() || !isTrialSpawner(block)) return Optional.empty();
    ...
    private void apply(TrialSpawnerState state, Block block, boolean placing, Runnable onApplied) {
        ...
        TrialSpawnerState applied =
                placing && !config.TRIAL_SPAWNER_CARRY_COOLDOWN.get() ? state.withCooldownRemaining(0) : state;
        platform.runTaskLater(
                block.getLocation(),
                () -> {
                    try {
                        if (!isTrialSpawner(block)) return;
                        trialSpawnerAdapter.applyState(block, applied);
                        onApplied.run();
                    } finally {
                        editedSpawners.endEdit(block.getLocation());
                    }

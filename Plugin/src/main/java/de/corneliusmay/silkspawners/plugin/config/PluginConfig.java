...
public enum PluginConfig {
    ...
    MESSAGE_LOCALE(builder(MESSAGES, "locale")
            .def("en")
            .formatter((ConfigValueFormatter<Locale>) Locale::forLanguageTag)
    ...
    SPAWNER_ITEM_PREFIX(builder(SPAWNER_ITEM, "prefix")
            .def("$e")
            .formatter(value -> value.isEmpty() ? "§f" : new MessageConfigValue().format(value))),
    SPAWNER_ITEM_PREFIX_OLD(builder(SPAWNER_ITEM, "prefixOld")
            .def("")
            .formatter(new MessageConfigValue())
            .legacy(SPAWNER_ITEM.getPath() + "prefix-old")),
    SPAWNER_ITEM_LORE(builder(SPAWNER_ITEM, "lore")
            .def(new String[0])
            .formatter(new MessageConfigValue())
            .list()),
    SPAWNER_EXPLOSION_ALL(
            builder(SPAWNER_EXPLOSION, "all").def(List.of()).formatter(new ExplosionTierListConfigValue())),
    SPAWNER_EXPLOSION_NORMAL(builder(SPAWNER_EXPLOSION, "normal")
            .def(List.of())
            .formatter(new ExplosionTierListConfigValue())
            .migrator(3, new ExplosionLegacyPowerMigrator())),
    SPAWNER_EXPLOSION_SILKTOUCH(builder(SPAWNER_EXPLOSION, "silktouch")
            .def(List.of())
            .formatter(new ExplosionTierListConfigValue())
            .migrator(3, new ExplosionLegacyPowerMigrator())),
    SPAWNER_MESSAGE_DENY_DESTROY(
            builder(SPAWNER_MESSAGES, "denyDestroy").def(true).formatter(new BooleanConfigValue())),
    SPAWNER_MESSAGE_DENY_PLACE(builder(SPAWNER_MESSAGES, "denyPlace").def(true).formatter(new BooleanConfigValue())),
    SPAWNER_MESSAGE_DENY_CHANGE(
            builder(SPAWNER_MESSAGES, "denyChange").def(true).formatter(new BooleanConfigValue())),
    SPAWNER_PERMISSION_DISABLE_DESTROY(
            builder(SPAWNER_PERMISSIONS, "disableDestroy").def(false).formatter(new BooleanConfigValue())),
    SPAWNER_PERMISSION_DISABLE_PLACE(
            builder(SPAWNER_PERMISSIONS, "disablePlace").def(false).formatter(new BooleanConfigValue())),
    SPAWNER_PERMISSION_DISABLE_CHANGE(
            builder(SPAWNER_PERMISSIONS, "disableChange").def(false).formatter(new BooleanConfigValue())),
    UPDATE_CONFIG_VERSION(
            builder(UPDATE, "configVersion").def(PluginConfig.CONFIG_VERSION).formatter(new IntegerConfigValue())),
    ...
    public void init(FileConfiguration config, Integer initialVersion) {
        ...
        if (initialVersion < CONFIG_VERSION && !migrators.isEmpty()) {
            ...
            for (List<ConfigValueMigrator> versionMigrators :
                    migrators.tailMap(initialVersion, false).values()) {
